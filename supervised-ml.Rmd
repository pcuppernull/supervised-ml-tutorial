...
The "Accuracy" measure that is returned is from the cross-validated data. To test the prediction accuracy on the holdout data, below I use the model to generate new predictions, compare them to the "true" values from the testing data, and see how often we got the prediction right.

```{r}
1 - sum(abs((as.numeric(predict(logit_fit, testing)) - 1) - (as.numeric(testing$violence_ok) - 1))) / nrow(testing)
```

So, the logit model makes the correct prediction 87% of the time on the testing data. Not bad!

### Elastic Net

Elastic Net is another common learner type. One hyperparameter of elastic net models is "Alpha" - `Caret` automatically runs several iterations of the elastic net model with different values for Alpha. It then shows you the accuracy at each level and will sugggest the most accurate model.

```{r}
elastic_fit <- train(violence_ok ~ ., #same as before
                   method = "glmnet", #specify the model type here
                   data = training, #same as before
                  #tuneLength = 10 # You can use this argument to test more options of hyperparameter value
...
We're going to do some "hyperparameter tuning", where we test out different values of alpha and lambda, and test each model on the hold out data. The code below makes it a little easier to generate predictions and plot the out-of-sample accuracy.
...
K Nearest Neighbors, or kNN, is another model type that is somewhat common. To make a classification under kNN, the learner looks for the k most similar observations in the training set to the testing observation on hand. Whichever is the most popuular outcome ofthe k most similar training observations is designated as the prediction for the test observation.

Choosing k is tricky. k of 1 might be too low because you are making a classification based on a single training observation! a k value that is too much might lose some of the nuance in the data.
