...
Supervised learning uses both the inputs and outputs of a data generating process to teach a computer to predict the value of an outcome variable. As a researcher, you provide the computer with both independent variables (the inputs) and dependent variables (the outputs), and the computer figures out how that inputs relate to each other to produce the outputs. Once this model -- or _learner_ -- has been "trained" by your data, you can apply it to new data to generate predictions. 
...
So, the logit model makes the correct prediction 87% of the time on the testing data. Not too bad!
...
```
kNN performs better than both the logit and elastic net models (aboout 88.6% out-of-sample prediction accuracy.)
